...
- Классы ДОЛЖНЫ именоваться используя `«CapitalizedCamelCase»` (каждое слово начинается с большой буквы, между словами нет разделителей)
...
- Объекты классов ДОЛЖНЫ именоваться используя `«concreteCamelCase»` (со строчной буквы)
...
- Имена методов ДОЛЖНЫ именоваться используя `«camelCase»` (первое слово пишется в нижнем регистре, далее каждое слово начинается с большой буквы, а между словами нет разделителей).
...
- Во всех уместных случаях вместо `foreach` РЕКОМЕНДУЕТСЯ использовать соотвутствующие методы обработки массивов
